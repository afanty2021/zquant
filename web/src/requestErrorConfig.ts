...
export const errorConfig: RequestConfig = {
  ...
  errorConfig: {
    ...
    errorThrower: (res) => {
      ...
      if (!success) {
        // 识别 FastAPI 响应格式：FastAPI 使用 message 字段，而不是 errorMessage
        // FastAPI 格式: { success: false, message: "...", data: [...] }
        // 标准错误格式: { success: false, errorMessage: "...", errorCode: ... }
        const resAny = res as any;
        if (resAny.message && !errorMessage) {
          // 这是 FastAPI 格式的响应，不应该抛出异常
          // 让响应正常返回，由业务代码处理 success=false 的情况
          return; // 不抛出异常，让响应正常返回
        }
        // 标准错误格式，抛出异常
